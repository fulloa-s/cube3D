...
/*   Updated: 2021/04/09 13:30:12 by fulloa-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	floor_ceiling(t_win_data *win)
{
	...
	while (win->rayc.y < win->resy)
	{
		...
		while (win->rayc.x < win->resx)
		{
			...
			win->rayc.tx = (int)(win->text[5].w * \
			(win->rayc.floorx - win->rayc.cellx)) & (win->text[5].w - 1);
			win->rayc.ty = (int)(win->text[5].h * \
			(win->rayc.floory - win->rayc.celly)) & (win->text[5].h - 1);
			win->rayc.floorx += win->rayc.floorstepx;
			win->rayc.floory += win->rayc.floorstepy;
			win->rayc.color = (unsigned int)win->text[5].addr[win->text[5].w \
			 * win->rayc.ty + win->rayc.tx];
			draw_dot(win, win->rayc.x, win->rayc.y, win->rayc.color);
			win->rayc.color = (unsigned int)win->text[6].addr[win->text[6].w \
			 * win->rayc.ty + win->rayc.tx];
